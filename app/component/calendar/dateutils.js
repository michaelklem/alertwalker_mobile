...
function weekDayNames(firstDayOfWeek = 0) {
  let dayNamesShort = XDate.locales[XDate.defaultLocale].dayNamesShort;
  let formattedDayNames = [];
  console.log(dayNamesShort);
  for(let i = 0; i < dayNamesShort.length; i++)
  {
    formattedDayNames.push(dayNamesShort[i].substr(0,1));
  }
  const dayShift = firstDayOfWeek % 7;
  if (dayShift) {
    formattedDayNames = formattedDayNames.slice(dayShift).concat(formattedDayNames.slice(0, dayShift));
  }
  return formattedDayNames;
}
